...
class _XmlAppBarBuilder extends CommonWidgetBuilder {
  ...
  Widget build(BuildContext buildContext, AssembleElement element, List<AssembleChildElement> descendant) {
    ...
    return AppBar(
      ...
      titleSpacing: res.size(attrs['titleSpacing']),
      toolbarOpacity: res.size(attrs['toolbarOpacity']) ?? 1.0,
      bottomOpacity: res.size(attrs['bottomOpacity']) ?? 1.0,
      toolbarHeight: res.size(attrs['toolbarHeight']),
      leadingWidth: res.size(attrs['leadingWidth']),
